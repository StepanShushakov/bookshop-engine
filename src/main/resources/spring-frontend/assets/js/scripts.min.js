        ...
        var Card = function () {
            var bookTemplate = function (book) {
                return '<div class="Card">' + '   <div class="Card-picture">' + '<a href="/books/' + book.slug + '">' + '       <img src="' + book.image + '">\n' + '</a>' +
                    ...
                    (book.discountPrice ? '<span class="Card-priceOld">₽' + book.price + '</span>\n' : '') + '                       <span class="Card-price">₽' + (book.discountPrice ? book.discountPrice : book.price) +
                    '</span>\n' + '                </div>\n' + '              </div>\n' + '            </div>';
            }
            return {
                init: function () {
                    $('[data-refreshshow]').on('click change', function (e) {
                            ...
                            $.ajax({
                                ...
                                complete: function (result) {
                                    if (result.status === 200) {
                                        ...
                                        // if ($this.data('refreshoffset') * $this.data('refreshlimit') >= result.responseJSON.count) {
                                        //     $('.Cards-footer .btn.btn_primary')["hide"]();
                                        // } else {
                                        //     $('.Cards-footer .btn.btn_primary')["show"]();
                                        // }
                                        var $Cards = $('.Cards_refresh');
                                        if (type === 'changedate') {
                                            $Cards.find('.Card').remove();
                        ...
                        switch (data) {
                            ...
                            case'search':
                                ...
                                getData('/search/page/' + $this.data('refreshid'), {
                                    offset: function () {
                                        var oldOffset = $this.data('refreshoffset')
                                        if(oldOffset < 1) {
                                            //$this.data('refreshoffset', oldOffset + $this.data('refreshlimit'))   //todo: this for google books api
                                            $this.data('refreshoffset', ++oldOffset)
                                        }else {
                                            // $this.data('refreshoffset', oldOffset);                              //todo: this for google books api
                                            $this.data('refreshoffset', ++oldOffset - $this.data('refreshlimit'));
                                        }
                                        return $this.data('refreshoffset')
                                    },
                                    ...
                                    if ($(slick.$slider).data('load')) {
                                        ...
                                        switch ($(slick.$slider).data('load')) {
                                            ...
                                            case'popular':
                                                ...
                                                break;
                                            case 'author':
                                                sendData('/author/' + $this.data('refreshid'))
                                                break;
                                        }
                                    }
                                }
        ...
        var Transactions = function () {
            var transactionTemplate = function (transaction) {
                ...
                return '<tr>\n' + '                      <td>' +
                    ...
                    date.getFullYear() + ' ' +
                    date.getHours() + ':' +
                    (date.getMinutes() < 10 ? '0' : '') + date.getMinutes() + '                      </td>\n' + '                      <td><span class="Transactions_textSuccess">\n' + '                          <strong>'
                    + (transaction.value > 0 ? '+' : '') + transaction.value + ' р.\n' + '                          </strong></span>\n' + '                      </td>\n' + '                      <td>' + transaction.description + '                      </td>\n' + '                    </tr>';
            }
